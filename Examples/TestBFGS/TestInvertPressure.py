...
def FromStressToGrad(uc, stress):
    ...
    for i in range(3):
        for j in range(3):
            for x in range(3):
                for y in range(3):
                    du_deps[3*i+j,3*x+y] = 0.5 * (uc[i,y] * I[x,j] + uc[i,x] * I[y,j])

    print du_deps
    deps_du = np.linalg.inv(du_deps)
    grad_mat = deps_du.dot ( stress.ravel())
    return grad_mat.reshape( (3,3))

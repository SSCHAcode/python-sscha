...
d3 = np.zeros((lanc.n_modes, lanc.n_modes, lanc.n_modes))
...
for k in range(lanc.n_modes):
    ...
    w2 = np.linalg.eigvals(lanc.L_linop)
...
plt.plot(w_array, spectral_lanczos_ft, label = "Lanczos continued frac.",
         ls = "-.")
plt.plot(w_array, -np.imag(green_function(w_array, lanc, eps)), ls  = "dotted", label = "Many body", color =  "k")

...
def test_all_temperature_lanczos(verbose = False):
    ...
    for i in range(len(files)):
        if verbose:
            print("Simulating T = {} | N_configs = {}".format(T[i], N_confs[i]))
        get_lanczos(T[i], N_confs[i], verbose)


def get_lanczos(T, N_CONF, verbose):
    directory = "T_{}_N_{}".format(T, N_CONF)
    lanc_dir = os.path.join(directory, "lanczos_noc")
    hessian_file = os.path.join(directory, "SnTe_hessian")
    save_file = os.path.join(directory, "green_function_noc.dat")
    ...
    # Setup the calculation by computing once for all the L operator
    # [THIS IS THE DIFFERENCE WITH THE OTHER TEST]
